...
import com.iscas.JavaExP.model.analyzeModel.ConditionTrackerInfo;
import com.iscas.JavaExP.model.analyzeModel.ConditionWithValueSet;
import com.iscas.JavaExP.model.analyzeModel.ExceptionInfo;
import com.iscas.JavaExP.model.analyzeModel.RefinedCondition;
...
//    }
    ...
    private static void addPreConditions(JSONObject jsonObject, ExceptionInfo exceptionInfo) {
        ...
        ArrayList<String> keyCondList= new ArrayList<>();
        if(exceptionInfo==null) return;
        // 将数组添加到JSON对象中
...
//                    "when executing the statements from " + trap.getBeginUnit() + " to " + trap.getEndUnit());
        ...
        for (ConditionWithValueSet conditionWithValueSet : exceptionInfo.getConditionTrackerInfo().getConditionWithValueSetMap().values()) {
        ...
        if (!exceptionInfo.isRethrow() && preCondList.size() == 0 &&
                exceptionInfo.getConditionTrackerInfo().getConditions().size() == 0 && exceptionInfo.getConditionTrackerInfo().getCaughtValues().isEmpty()) {
        ...
        ConditionWithValueSet conditionWithValueSet = exceptionInfo.getConditionTrackerInfo().getKeyConditionWithValueSet();
        if (conditionWithValueSet!=null && conditionWithValueSet.toString().length() > 0){
            for(RefinedCondition refinedCondition: conditionWithValueSet.getRefinedConditions()) {
                if (!keyCondList.contains(refinedCondition.toString())) {
                    if (refinedCondition.getLeftStr().length() == 0)
                        refinedCondition.setLeftStr("sth.");
                    if (refinedCondition.getRightStr().length() == 0)
                        refinedCondition.setRightStr("sth.");
                    keyCondList.add(refinedCondition.toString());
                }
            }
        }
        if (keyCondList.size() > 0) {
            jsonObject.put("keyPreCondition", keyCondList.get(0));
        }
